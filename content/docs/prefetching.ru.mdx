...
## Предварительная выборка в React Server Components (RSC) [#pre-fetch-in-rsc]

При использовании Next.js App Router с React Server Components вы можете предварительно получить данные на сервере и передать их клиентским компонентам через `SWRConfig`. Этот подход позволяет вам использовать выборку данных на стороне сервера, сохраняя при этом преимущества кеширования и ревалидации на стороне клиента SWR.

Для получения подробной информации и примеров см. документацию [Использование с Next.js](/docs/with-nextjs#prefetch-data-in-server-components).
