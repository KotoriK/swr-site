...
## React Server Components (RSC)에서 프리페치 [#pre-fetch-in-rsc]

Next.js App Router와 React Server Components를 사용할 때, 서버에서 데이터를 프리페치하고 `SWRConfig`를 통해 클라이언트 컴포넌트로 전달할 수 있습니다. 이 접근 방식을 통해 SWR의 클라이언트 측 캐싱 및 재검증의 이점을 유지하면서 서버 측 데이터 페칭을 활용할 수 있습니다.

자세한 정보와 예제는 [Next.js와 함께 사용하기](/docs/with-nextjs#prefetch-data-in-server-components) 문서를 참조하세요.
