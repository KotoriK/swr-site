...
## Pre-fetch en React Server Components (RSC) [#pre-fetch-in-rsc]

Al usar Next.js App Router con React Server Components, puedes precargar datos en el servidor y pasarlos a componentes del cliente a través de `SWRConfig`. Este enfoque te permite aprovechar la obtención de datos del lado del servidor mientras mantienes los beneficios del almacenamiento en caché y revalidación del lado del cliente de SWR.

Para obtener información detallada y ejemplos, consulta la documentación de [Uso con Next.js](/docs/with-nextjs#prefetch-data-in-server-components).
