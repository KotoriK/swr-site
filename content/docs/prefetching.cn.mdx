...
## 在 React Server Components (RSC) 中预获取 [#pre-fetch-in-rsc]

当使用 Next.js App Router 和 React Server Components 时，你可以在服务器上预获取数据，并通过 `SWRConfig` 将其传递给客户端组件。这种方法允许您利用服务器端数据获取，同时保持 SWR 客户端缓存和重新验证的优势。

详细信息和示例请参见 [与 Next.js 一同使用](/docs/with-nextjs#prefetch-data-in-server-components) 文档。
