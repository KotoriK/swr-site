...
## Pré-obtenção em React Server Components (RSC) [#pre-fetch-in-rsc]

Ao usar o Next.js App Router com React Server Components, você pode pré-obter dados no servidor e passá-los para componentes de cliente via `SWRConfig`. Essa abordagem permite que você aproveite a busca de dados do lado do servidor enquanto mantém os benefícios do cache e revalidação do lado do cliente do SWR.

Para informações detalhadas e exemplos, consulte a documentação [Usando com Next.js](/docs/with-nextjs#prefetch-data-in-server-components).
